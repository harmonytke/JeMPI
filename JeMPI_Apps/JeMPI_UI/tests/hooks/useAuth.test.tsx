...
describe('useAuth', () => {
  ...
  const mockApiClient = (methodsDic: { [methodName: string]: any }) => {
    for (const [methodName, mockMethod] of Object.entries(methodsDic)) {
      jest
        .spyOn(apiClient, methodName as keyof typeof apiClient)
        .mockImplementation(mockMethod)
    }
  }

  type KeycloakMethod = (...args: any[]) => any
  const mockKeyClock = (methodsDic: {
    [methodName: string]: KeycloakMethod
  }) => {
    for (const [methodName, mockMethod] of Object.entries(methodsDic)) {
      jest
        .spyOn(keycloakInstance, methodName as keyof typeof keycloakInstance)
        .mockImplementation(mockMethod as any)
    }
  }
      ...
      return (
        ...
        >
          <QueryClientProvider client={queryClient}>
            <AuthProvider>{ChildComponent}</AuthProvider>
          </QueryClientProvider>
        </SnackbarProvider>
      )
    }
  ...
  describe('Auth Checker tests', () => {
    ...
    it('Shows loading if use details are still loading', async () => {
      ...
      expect(
        renderComponent.container.querySelector('#user-loading-spinner')
      ).toBeTruthy()
    })
